...
public class MainForm extends JFrame {
	...
	public void connect() {
		...
		try {
			...
			new Thread(new ClientThread(socket, MainForm.this)).start();
	...
	private void handleClientRequest(Socket clientSocket) {
	    try (ObjectOutputStream outputStream = new ObjectOutputStream(clientSocket.getOutputStream());
	        ...
	        // Lấy danh sách tin nhắn gần đây từ server
	        List<Message> recentMessages = (List<Message>) inputStream.readObject();

	        // Hiển thị danh sách tin nhắn trên jTextPane1
	        for (Message message : recentMessages) {
	            appendMessage(message.getDateTime() + " - " + message.getSender() + ": " + message.getContent(), "Tin nhắn", Color.BLACK, Color.BLACK);
	        }
		...
		sendFileMenu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!attachmentOpen) {
					...
					if (s.prepare(username, host, port, MainForm.this)) {
		...
		jMenuItem3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					JFileChooser chooser = new JFileChooser();
					chooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
					int browse = chooser.showOpenDialog(MainForm.this);
